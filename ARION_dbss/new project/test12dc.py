...
encrypted_combined = parts[0].encode()
lengths = list(map(int, parts[1:6]))
cipher_key = parts[6].encode()

print(f"Encrypted combined data: {encrypted_combined}")
print(f"Lengths: {lengths}")
print(f"Cipher key: {cipher_key}")

# Extract lengths of encrypted parts and public key
len_a, len_b, len_c, len_d, len_pk = lengths

# Decrypt the combined string using AES
cipher = Fernet(cipher_key)
combined_encrypted = cipher.decrypt(encrypted_combined).decode()

# Extract encrypted parts using lengths
start = 0
encrypted_a = float(combined_encrypted[start:start + len_a])
start += len_a
encrypted_b = float(combined_encrypted[start:start + len_b])
start += len_b
encrypted_c = float(combined_encrypted[start:start + len_c])
start += len_c
encrypted_d = float(combined_encrypted[start:start + len_d])
start += len_d
public_key = int(combined_encrypted[start:start + len_pk])
